 ...
 * @author ASUS
 */
public class ListCorbeilleForm extends BaseForm{
    
    Form current;
    
    
    
    public ListCorbeilleForm(Resources res){
    ...
    cnt.addAll(btnAddRecs); super.add(cnt);
        ...
        Corbeille.setSelected(true);
        arrow.setVisible(false);
        addShowListener(e -> { arrow.setVisible(true); updateArrowPosition(Corbeille, arrow); });
        ...
        ArrayList<Reclamation>list = ServiceReclamation.getInstance().affichageRC(42);
    ...
    private void addButton(Reclamation rec,Resources res) {
       ...
       Label etat=new Label("Etat :"+rec.getEtat(),"NewsTopLine2");
        ...
        else{  dig.dispose(); if(ServiceReclamation.getInstance().deleteReclamation(rec.getId())){ new ListCorbeilleForm(res).show(); }}   });
